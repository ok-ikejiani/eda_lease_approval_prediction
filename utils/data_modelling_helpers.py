...
def train_modelsx(model_list: list[ModelWrapper], pipeline:Pipeline, X_train=None, y_train=None, cv=5, scoring=None):
    """
...
def evaluate_modelsold(best_models, X_test, y_test, model_type='classification', scoring='accuracy'):
    """
    Evaluates the best models on the test set and displays a table with the model performance.
    Returns only the best model of each type based on the scoring parameter.
    Creates charts based on model type (classification or regression).
    ...
    # Select the model with the best value for the specified scoring metric
    if model_type == 'classification':
        if scoring == 'accuracy':
            best_model_df = df.loc[df['Accuracy'].idxmax()]
        elif scoring == 'precision':
...
# def evaluate_models(best_models, X_test, y_test, model_type='classification'):
    """
    Evaluates the best models on the test set and displays a table with the model performance.
    Creates charts based on model type (classification or regression).
